...
```bash
SKILL_DIR="$(dirname "$(find ~/.claude -path '*/skills/execute/driver.mjs' -print -quit 2>/dev/null || find . -path '*/skills/execute/driver.mjs' -print -quit 2>/dev/null)")"
```

If that fails, use Glob to search for `**/skills/execute/driver.mjs` and extract the directory.
